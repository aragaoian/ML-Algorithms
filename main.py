...
def regressions(df_path: str = None, validation_df_path: str = None, target: str = "y"):
    ...
    em_elastic_net = EvaluationMetrics(
        n=X_test.shape[0], y=y_test, y_pred=y_pred_elastic_net
    )
    ...
    print(
        f"ElasticNet Regression - MAE: {em_elastic_net.mae()}, MSE: {em_elastic_net.mse()}, RMSE: {em_elastic_net.rmse()}, R2: {em_elastic_net.rsquared()}"
    )
